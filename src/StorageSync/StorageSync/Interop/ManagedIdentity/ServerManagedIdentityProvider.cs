...
namespace Microsoft.Azure.Commands.StorageSync.Interop.ManagedIdentity
{
    ...
    {
        private IServerManagedIdentityTokenProvider serverMITokenProvider;
        ...
        public ServerType GetServerType(IEcsManagement ecsManagement)
        {
            ...
            if (!EnableMIChecking)
            {
                return ServerType.HybridServer;
            }
        ...
        public Guid GetServerApplicationId(ServerType serverType, bool throwIfNotFound = true, bool validateSAMI = true)
        {
            ...
            if (EnableMIChecking)
            {
                try
                {
                    if (serverType == ServerType.HybridServer)
                    {
                        return applicationId;
                    }

                    serverMITokenProvider = serverMITokenProvider ?? new ServerManagedIdentityTokenProvider(serverType, traceLog: this.TraceLog);

                    // We need to use the https://storage.azure.com resource, as this provides us the x-ms-rid header to use for validation
                    var token = Task.Run(() => serverMITokenProvider.GetManagedIdentityAccessToken(resource: "https://storage.azure.com/")).GetAwaiter().GetResult();
        ...
        public void Dispose()
        {
            this.serverMITokenProvider?.Dispose();
        }
    }
